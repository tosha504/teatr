(function () {
  ...
  jQuery(window).scroll(function () {
    ...
    jQuery(".calendar").css({
      position: "sticky",
      top: `${document.querySelector(".header").clientHeight - 1}px`,
      "z-index": "99999",
    });
    if (scrollTop > 40) {
  ...
  if (arr) {
    ...
    const callback = function (entries, observer) {
      entries.forEach((entry) => {
        ...
        if (isIntersecting) {
          arr.style.cssText += `position: sticky;top:${
            document.querySelector(".header").clientHeight - 1
          }px;background-color:white;z-index:99999`;
        }
      });
    };
  ...
  jQuery(".shows-list__data li button").on("click", function (e) {
    const attrName = jQuery(e.target).attr("name");
    if (attrName === "filter_month") {
      jQuery("#filter_month").val(e.target.value);
    }
  });
  ...
  jQuery(".people__categories li a").on("click", function (e) {
    e.preventDefault();
    let slug = jQuery(e.target).attr("data-term-slug");
